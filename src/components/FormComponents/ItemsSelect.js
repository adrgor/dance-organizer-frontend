...
export default function ItemsSelect({items, isCheckboxSelected, isSearchEnabled, label, setValue, selectedItems, style}) {
  ...
  return (
    <div ref={useOutsideClick(handleOnClickOutside)}>
        <button class="mb-5 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-full p-2.5 flex items-center justify-between" type="button" onClick={() => setIsListDisplayed(!isListDisplayed)}>{selectedItem}<svg class="w-4 h-4 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg></button>
        {isListDisplayed &&
